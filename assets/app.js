...
1. 输出与章节数完全一致的 JSON 数组，顺序对应每一章：{"chapterPlans":["-”]}。
2. 禁止输出节奏和埋点/回收之外的其他内容。
2. 全局埋点与回收规划：先建立全书埋点清单，明确埋点章节、回收章节、关联逻辑。只在出现埋点或回收的章节梗概内写出该埋点或回收的详细剧情，且必须写清具体情境、对话或线索。完成后自查埋点与回收是否形成闭环，无遗漏、无悬空。埋点被回收的时候，必须写清楚前面那个埋点详情，还必须写清楚是如何回收的埋点。
