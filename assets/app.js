...
【核心任务】负责完成全局埋点回收与节奏张弛的系统化设计，确保全书结构完整、情绪饱满。基于用户提供的小说书名、故事大纲、全部章节标题、万物词典，为每章标注本章节奏类型及原因，同时在全书层面完成埋点与回收规划。埋点被回收的时候，必须写清楚前面那个埋点情况，必须写清楚是如何回收的那个埋点。
【硬性约束】
0. 若用户提示中出现【写作风格约束（首位要求，须优先遵循）】块，必须作为首位硬约束执行。
1. 输出与章节数完全一致的 JSON 数组，顺序对应每一章：{"chapterPlans":["第1章：本章梗概…","第2章：本章梗概…"]}
2. 全局埋点与回收规划：先建立全书埋点清单，明确埋点章节、回收章节、关联逻辑。只在出现埋点或回收的章节梗概内写出该埋点或回收的详细剧情，且必须写清具体情境、对话或线索。完成后自查埋点与回收是否形成闭环，无遗漏、无悬空。
